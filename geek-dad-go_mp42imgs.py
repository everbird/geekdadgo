...
custom_config = '--oem 3 --psm 11 -c tessedit_char_whitelist={} -c tessedit_char_blacklist={}'.format(whitelist, blacklist)


def get_date_string(frame):
    x, y = 20, 10
    w, h = 120, 50
    ...
    _text = pytesseract.image_to_string(gray, lang='eng', config=custom_config)
    _text = _text.replace(" ", "")
...
def run():
    ...
    while i < frame_count:
        ...
        if 253 <= r <= 255 and 250 <= g <= 253 and 224 <= b <= 247:
            ...
            ds = get_date_string(cropped_frame)
            ds = ds.replace(" ", "")
            if len(ds) < 5:
            ...
            pre = ds
            cv2.imwrite(f'images/img-frame{i:04d}-{ds}.png', cropped_frame)
